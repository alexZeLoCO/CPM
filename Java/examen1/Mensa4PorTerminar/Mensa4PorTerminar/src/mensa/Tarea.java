...
public abstract class Tarea implements SincroForeBack{
    ...
    public void Run(){
        ...
        if(promo.length()>0){
            Random R=new Random(promo.hashCode());
            float descuento=0;
            Color C=new Color(255,0,0);
            for(int i=0;i<50;i++){
                Duerme(0.1F);
                descuento=((int)((descuento+R.nextFloat())*100))/100.0F;
                int c=(int)(descuento/50*255);
                C=new Color(255-c,c,0);
                infoPromo(descuento,descuento/50,C);
            }
        }
