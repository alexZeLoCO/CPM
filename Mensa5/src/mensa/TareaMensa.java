...
public class TareaMensa extends Tarea{
    ...
    private boolean IsStopped;	// He buscado esto
    private boolean isPaused;	// He incluido esto
    
    public TareaMensa(Envio VEnvio){
        super();
        this.VEnvio=VEnvio;
        this.IsStopped=false;	// He buscado esto
	this.isPaused = false;	// He incluido esto
    }
    ...
    public void Start(){ // Empezó 
        ...
        this.VEnvio.bPararCalculo.setEnabled(true);	// He buscado esto

 	this.VEnvio.BT_Pause.setEnabled(true);	// He incluido esto
    ...
    public boolean Stop(){ // Si retorna verdadero la tarea debe intentar terminar
        return IsStopped;			// He buscado esto
    }
    
    @Override
    public boolean pause() {
	    return this.isPaused;	// He incluido esto
    }

    public void setStop(){	// He buscado esto
        IsStopped=true;
        this.VEnvio.bCalcularImporte.setEnabled(true);
        this.VEnvio.bPararCalculo.setEnabled(false);
        this.VEnvio.bRealizarPago.setEnabled(false);
    }
	public void setPause (boolean b) {	// He incluido esto
		this.isPaused = b;
		this.VEnvio.BT_Pause.setEnabled(!b);	// Desactivo botón pausa
		this.VEnvio.BT_Continue.setEnabled(b);	// Activo botón reanudar
	}    
